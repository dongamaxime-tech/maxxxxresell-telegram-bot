...
const ACCOUNTS = {
  'admin': { password: process.env.ADMIN_PASSWORD || 'Maxdonga!', role: 'admin' },
  'charles': { password: process.env.CHARLES_PASSWORD || 'charlou', role: 'user' }
};
...
async function handleMessage(msg) {
  ...
  if(session.state === 'await_password') {
    ...
    var password = text;
    var acc = getAccountByCredentials(username, password);
    if(acc) {
...
function getAccountByCredentials(username, password) {
